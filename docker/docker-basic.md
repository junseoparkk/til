...
### 4) 도커의 용도
1. 동일한 개발 환경 제공이 필요한 경우
    - 여러 개발자에게 동일한 개발환경을 제공할 수 있음
    - 예를 들어 프로젝트별로 컨테이너를 따로 사용할 수 있음
    - 개발환경과 운영환경의 차이가 사라짐
2. 격리된 환경을 통해 새로운 버전의 테스트가 필요한 경우
    - 운영체제나 라이브러리 등의 새로운 버전을 개발환경에서 테스트한 뒤 운영환경에 적용할 수 있음
    - 새로운 버전 뿐만 아니라 변경된 환경에 대한 테스트시에도 유용
3. 동일한 서버가 여러 대 필요한 경우
    - 동일한 서버가 여러 개 필요한 경우 컨테이너를 이용해 한 대의 물리 서버에 서버를 여러 대 만들 수 있음
    - 물리 서버를 여러 개의 컨테이너가 공유하므로 비용 절감 가능
    - 스케일링에 유리하므로 웹 서버나 API 서버로 활용하는 등 다양한 활용 방법이 존재