...
- 애플리케이션을 '컨테이너' 단위로 패키징하여 배포가 가능. 각 컨테인너는 독립적이며 Host OS 와는 격리됨
<br><br>

### Q. CI/CD 란?
- CI(Continuous Integrity)는 지속적인 통합, CD(Continuous Deployment)는 지속적인 배포를 말함
- 애플리케이션 빌드부터 배포까지 과정을 자동화하는 과정
- ex) Github로 코드를 커밋하면 Jenkins와 같은 자동화 빌드 도구가 이를 감지하고 자동으로 빌드하도록 설계
<br><br>
